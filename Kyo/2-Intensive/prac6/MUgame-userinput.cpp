...
/*
 * This main function will prompt the user to key in the size of the string then request user to key in the string, subsequently prompt the user which rule or not rule they wish the application to run.
 * Once the user made their choice, the program will run in accordance to the rule's rulebook. Then return the user with the transformed string after it had went through the rule
 */
int main() 
{
	//Task 2
	...
	//To check if the string is a valid for MU.
		if(isaMUString(word, sizeOfSequence))
		{
			...
			{
				cout << "Sorry, as rule 1 cannot be applied due to unmet condition! Therefore the word remain as "<<word<<endl;
			}
		}
		else
		{
	...
	else if (choice == 2) 
	{
		...
		{
			cout << "Sorry, as rule 2 cannot be applied due to unmet condition! Therefore the word remain as "<<word<<endl;
		}
	}		
