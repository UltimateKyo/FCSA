...
int main() {
    ...
    while (myfileIn >> characters) { //read until end of file
        ...
        if (choice == 1) {
            myfileOut << "Character(s) entered: "<< sequence <<endl;
            
            char* newSequence = new char[size+1];
            newSequence = applyRule1(sequence,size);
            
            //cout << newSequence;
            myfileOut << "Character(s) AFTER applying Rule: "<< newSequence <<endl;
        }
        else if (choice == 2) {
            ...
            newSequence  = applyRule2(sequence,size);
            ...
            myfileOut << "No Rule(s) was being applied. Good bye!"<<endl;
        }
        else {
            
            myfileOut << "Error! Invalid Choice!"<<endl;
        }
